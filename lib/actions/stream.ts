...
export async function getStreamUserToken() {
    ...
    const { data: userData, error: userError} = await supabase.from("user").select("full_name,avatar_url").eq("id",user.id).single();
    ...
    const serverClient = StreamChat.getInstance( process.env.NEXT_PUBLIC_STREAM_API_KEY!,
